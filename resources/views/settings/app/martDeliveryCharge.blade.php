    ...
    <div class="page-wrapper">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h3 class="text-themecolor">{{ trans('lang.mart_delivery_charge')}}</h3>
            </div>
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{url('/dashboard')}}">{{trans('lang.dashboard')}}</a></li>
                    <li class="breadcrumb-item active">{{ trans('lang.mart_delivery_charge')}}</li>
                </ol>
            </div>
        </div>
        <div class="card-body">
            ...
            <div class="row restaurant_payout_create">
                <div class="restaurant_payout_create-inner">
                    <fieldset>
                        <legend>{{trans('lang.mart_delivery_charge')}}</legend>
        ...
        @section('scripts')
            <script>
                ...
                var ref_deliverycharge = database.collection('settings').doc("martDeliveryCharge");

                $(document).ready(function() {
                    jQuery("#data-table_processing").show();

                    ref_deliverycharge.get().then(async function(snapshots_charge) {
                        var martDeliveryChargeSettings = snapshots_charge.data();

                        // Create default doc if not exists
                        if (!martDeliveryChargeSettings) {
                            await database.collection('settings').doc('martDeliveryCharge').set({
                        ...
                        try {
                            ...
                            if (martDeliveryChargeSettings.vendor_can_modify) {
                                $("#vendor_can_modify").prop('checked', true);
                            }

                            // Populate only if value exists and is not null/undefined
                            // Only populate once when the page is loaded
                            if (martDeliveryChargeSettings.minimum_delivery_charges !== undefined && martDeliveryChargeSettings.minimum_delivery_charges !== null && martDeliveryChargeSettings.minimum_delivery_charges !== '') {
                                $("#minimum_delivery_charges").val(martDeliveryChargeSettings.minimum_delivery_charges);
                            }
                            if (martDeliveryChargeSettings.minimum_delivery_charges_within_km !== undefined && martDeliveryChargeSettings.minimum_delivery_charges_within_km !== null && martDeliveryChargeSettings.minimum_delivery_charges_within_km !== '') {
                                $("#minimum_delivery_charges_within_km").val(martDeliveryChargeSettings.minimum_delivery_charges_within_km);
                            }

                            // New PDF fields
                            if (martDeliveryChargeSettings.base_delivery_charge !== undefined && martDeliveryChargeSettings.base_delivery_charge !== null) {
                                $("#base_delivery_charge").val(martDeliveryChargeSettings.base_delivery_charge);
                            }
                            if (martDeliveryChargeSettings.free_delivery_distance_km !== undefined && martDeliveryChargeSettings.free_delivery_distance_km !== null) {
                                $("#free_delivery_distance_km").val(martDeliveryChargeSettings.free_delivery_distance_km);
                            }
                            if (martDeliveryChargeSettings.per_km_charge_above_free_distance !== undefined && martDeliveryChargeSettings.per_km_charge_above_free_distance !== null) {
                                $("#per_km_charge_above_free_distance").val(martDeliveryChargeSettings.per_km_charge_above_free_distance);
                            }
                            if (martDeliveryChargeSettings.item_total_threshold !== undefined && martDeliveryChargeSettings.item_total_threshold !== null) {
                                $("#item_total_threshold").val(martDeliveryChargeSettings.item_total_threshold);
                            }
                        } catch(error) {
                            console.error('Error loading mart delivery charge settings:', error);
                        }
                    });
                    ...
                    $(".edit-setting-btn").click(function() {
                        ...
                        ref_deliverycharge.get().then(doc => {
                            if (doc.exists) {
                                ...
                                database.collection('settings').doc("martDeliveryCharge")
                                    .update(dataToUpdate)
                                    .then(function(result) {
                                        window.location.href = '{{ url("settings/app/martDeliveryCharge")}}';
                                    })
                                    .catch(function(error) {
                                        console.error('Error updating mart delivery charge settings:', error);
                                        $(".error_top").show();
