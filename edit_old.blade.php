    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            if (product.section) {
                ...
                if (product.hasOwnProperty('photo')) {
                    photo = product.photo;
                    if (photo != '' && photo != null) {
                        $(".product_image").append('<span class="image-item" id="photo_1">' +
                            '<span class="remove-btn" data-id="1" data-img="' + photo + '" data-status="old">' +
                            '<i class="fa fa-remove"></i></span>' +
                            '<img onerror="this.onerror=null;this.src=\'' + placeholderImage + '\'" class="rounded" width="50px" id="" height="auto" src="' + photo + '">' +
                            '</span>');
                    } else {
            ...
            $(".edit-form-btn").click( async function () {
                ...
                // photo is already set from the form
                if (name == '') {
                    $(".error_top").show();
                    $(".error_top").html("");
                    ...
                    uploadTask.on('state_changed', function (snapshot) {
                        ...
                        uploadTask.snapshot.ref.getDownloadURL().then(function (downloadURL) {
                            ...
                            $(".product_image").append(
                                ...
                                '<span class="remove-btn" data-id="1" data-img="' + downloadURL + '">' +
                                '<i class="fa fa-remove"></i></span>' +
                                '<img class="rounded" width="50px" id="" height="auto" src="' + downloadURL + '">' +
                                '</span>'
                            );
                        });
                    });
        ...
        async function storeImageData() {
            if (photo && photo.startsWith('data:image')) {
                // New image uploaded, upload to Firebase
        ...
        $("#product_image").resizeImg({
            callback: function (base64str) {
                ...
                $(".product_image").append(
                    '<span class="image-item" id="photo_1">' +
                    '<span class="remove-btn" data-id="1" data-img="' + base64str + '" data-status="new">' +
                    '<i class="fa fa-remove"></i></span>' +
